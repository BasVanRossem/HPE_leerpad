...
(function () {
  ...
  function showNavigatorImage() {
    ...
    const counterEl = document.getElementById("figureCounter");
    ...
    if (counterEl) {
      counterEl.textContent = `Figuur ${state.currentNavIndex + 1} van ${state.navigatorImages.length}`;
    }
  }
  ...
  function renderHotspotInteraction(block, opts) {
    ...
    btn.addEventListener("click", () => {
      ...
      zones.forEach((zone) => {
        ...
        const correct = !!chip && chipToLabel.get(chip) === zone.label;
        zEl.classList.toggle("zone-correct", correct);
        zEl.classList.toggle("zone-incorrect", !correct);
  ...
  function renderSortInteraction(block, opts) {
    ...
    shuffle(block.items || []).forEach((item) => {
      ...
      tray.appendChild(chip);
      makeDraggable(chip, ".sort-category-content, .chip-tray", tray, () => {});
    });
    ...
    btn.addEventListener("click", () => {
      let allCorrect = (block.items || []).length > 0;
      wrap.querySelectorAll(".drag-chip").forEach((chip) => {
        ...
        const correct = !!parentContent && parentContent.dataset.categoryId === item.categoryId;
        chip.classList.toggle("chip-correct", correct);
        chip.classList.toggle("chip-incorrect", !correct);
        if (!correct) allCorrect = false;
      });
